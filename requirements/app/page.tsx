...
function ScoreRow({id,answer}:{id:string;answer:Answer}) { const priority=answer.priorities[id]==="must"?"must":"should"; return <div className={`spec-row ${priority}`}><div><small>{item(id).category}</small><b>{item(id).label}</b></div><span className={`priority-badge ${priority}`}>{priority==="must"?"必ず":"できれば"}</span></div> }
